...
public abstract class GeneralRegion implements GeneralRegionInterface, Comparable<GeneralRegion>, ReplacementProvider {
    ...
    private transient boolean saveRequired = false;
    ...
    private void writeSettings() {
        ...
        setSetting("general.world", worldName);
     ...
     */
    public <T> void notifyAndUpdate(NotifyRegionEvent<T> event) {
     ...
     *
     * @return The UUID of the landlord of this region (nullable)
     */
    ...
    /**
     * Get the landlord of this region (the player that receives any revenue from this region) asynchronously.
     *
     * @return A future containing the The UUID (which may be null) of the landlord of this region
     */
    public CompletableFuture<UUID> getLandLordAsync() {
        String landlord = getStringSetting("general.landlord");
        if (landlord != null && !landlord.isEmpty()) {
            try {
                return CompletableFuture.completedFuture(UUID.fromString(landlord));
            } catch (IllegalArgumentException ignored) {
            }
        }
        String landlordName = getStringSetting("general.landlordName");
        ...
        if (landlordName != null && !landlordName.isEmpty()) {
            Bukkit.getScheduler().runTaskAsynchronously(plugin, () -> {
                OfflinePlayer offlinePlayer = Bukkit.getOfflinePlayer(landlordName);
                future.complete(offlinePlayer.hasPlayedBefore() ? offlinePlayer.getUniqueId() : null);
            });
        }
        return future;
    }
    ...
    public Object provideReplacement(String variable) {
        switch (variable) {
            ...
            case AreaShop.tagMinute:
                return Instant.now().get(ChronoField.MINUTE_OF_DAY);
            case AreaShop.tagHour:
                return Instant.now().get(ChronoField.HOUR_OF_DAY);
            case AreaShop.tagDay:
                return Instant.now().get(ChronoField.DAY_OF_MONTH);
            case AreaShop.tagMonth:
                return Instant.now().get(ChronoField.MONTH_OF_YEAR);
            case AreaShop.tagYear:
                return Instant.now().get(ChronoField.YEAR);
    ...
    /**
     * Check now if the player has been inactive for too long.
     */
    public boolean checkInactive() {
        return checkInactivity(Bukkit.getOfflinePlayer(getOwner()));
    }

    /**
     * Check now if the player has been inactive for too long.
     *
     * @param player The offline player instance of {@link #getOwner()}, used to prevent sync IO lookups.
     * @return Returns true if the player has been inactive for too long.
     */
    public abstract boolean checkInactivity(final OfflinePlayer player);

    /**
     * Handler for when the inactivity thereshold has been reached.
     *
     * @return Returns whether the operation was handled successfully.
     */
    public abstract boolean handleInactivity();

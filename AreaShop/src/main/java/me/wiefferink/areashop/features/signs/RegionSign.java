    ...
    public boolean update() {
        ...
        if (!Tag.SIGNS.isTagged(blockState.getType())) {
            ...
            if (!Materials.isSign(block.getType())) {
                final String message = String.format("Setting sign of $1%s of region $2%s failed, could not set sign block back", key, region.getName());
                errorLogger.submitWarning(message);
                return false;
            }
        } else {
            final String message = "Setting sign $1%s of region $2%s failed, RegionSign material was: $3%s";
            errorLogger.submitWarning(String.format(message, key, region.getName(), signType.name()));
        }

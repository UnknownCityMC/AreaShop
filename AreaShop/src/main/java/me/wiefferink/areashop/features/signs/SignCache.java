...
public class SignCache implements Listener {
    ...
    private final BlockBehaviourHelper behaviourHelper = AreaShop.getInstance().getNMSHelper().behaviourHelper();
    ...
    public void onIndirectSignBreak(BlockPhysicsEvent event) {
        ...
        AreaShop.debug("onIndirectSignBreak: Removed block of sign for", regionSign.getRegion().getRegionId(), "at", regionSign.getStringLocation());
    ...
    public void onSignChange(SignChangeEvent event) {
        ...
        if (line0.contains(plugin.getConfig().getString("signTags.rent"))) {
            ...
            FileManager.AddResult addResult = plugin.getFileManager().checkRegionAdd(player, regionManager.getRegion(secondLine), event.getPlayer().getWorld(), RegionType.RENT);
            ...
            FileManager.AddResult addResult = plugin.getFileManager().checkRegionAdd(player, region, event.getPlayer().getWorld(), RegionType.BUY);
            ...
            if (secondLine != null && !secondLine.isEmpty()) {
                ...
                List<LegacyGeneralRegion> regions = Utils.getImportantRegions(event.getBlock().getLocation());
                if (regions.isEmpty()) {
                    plugin.message(player, "addsign-noRegions");
                    return;
                } else if (regions.size() > 1) {
                    plugin.message(player, "addsign-couldNotDetectSign", regions.get(0).getRegionId(), regions.get(1).getRegionId());
                    return;
                }

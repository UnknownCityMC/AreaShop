...
public class Utils {
    ...
    public static <E> CompletableFuture<?> runAsBatches(final Collection<E> objects, final int batchSize, final Consumer<E> task, final boolean async) {
        ...
        if (async) {
            runnable.runTaskTimerAsynchronously(AreaShop.getInstance(), 1, 1);
        } else {
            runnable.runTaskTimer(AreaShop.getInstance(), 1, 1);
        }
        return future;
    }

    public static BukkitTask runCancellingTaskTimer(final Supplier<Boolean> task, final long delay, final long period, boolean async) {
        BukkitRunnable runnable = new BukkitRunnable() {
            @Override
            public void run() {
                if (!task.get()) {
        ...
        if(async) {
            return runnable.runTaskTimerAsynchronously(AreaShop.getInstance(), delay, period);
        } else {
            return runnable.runTaskTimer(AreaShop.getInstance(), delay, period);
        }
    }
